# Crisis Management Templates

## Overview

This document contains ready-to-use templates for crisis management planning and response. These templates should be customized for your organization's specific needs and context.

**Before using templates:**
1. Review `1-crisis-management-framework.md` to understand crisis management principles
2. Customize templates to your organization's structure and needs
3. Fill in contact information and procedures
4. Review and update quarterly

-----

## Crisis Management Plan Template

See the comprehensive template in `1-crisis-management-framework.md` Section 4: Crisis Management Plan Template.

-----

## Crisis Severity Assessment Template

```
CRISIS SEVERITY ASSESSMENT
─────────────────────────────────────

Date/Time: [Date/Time]
Assessed By: [Name, Role]
Crisis Type: [Security/Outage/Safety/etc.]

ASSESSMENT QUESTIONS:

1. Impact Scope:
☐ Single team/product affected
☐ Multiple teams/products affected
☐ Company-wide impact
☐ External stakeholders affected

2. Customer Impact:
☐ < 1% of users affected
☐ 1-10% of users affected
☐ 10-50% of users affected
☐ > 50% of users affected

3. Business Impact:
☐ Minimal financial impact
☐ Moderate financial impact (< $X)
☐ Significant financial impact
☐ Existential threat to business

4. Reputation Risk:
☐ Low media/social attention
☐ Moderate attention expected
☐ High attention expected
☐ Viral/regulatory attention

5. Timeline:
☐ Can be resolved within hours
☐ Requires days to resolve
☐ Requires weeks to resolve
☐ Ongoing/unclear timeline

SEVERITY LEVEL:
☐ Level 1 (Yellow) - Minor Crisis
☐ Level 2 (Orange) - Major Crisis
☐ Level 3 (Red) - Severe Crisis

RECOMMENDED ACTIONS:
[Based on severity level]

NEXT STEPS:
[Immediate actions to take]
```

-----

## Crisis Situation Log Template

```
CRISIS SITUATION LOG
─────────────────────────────────────

Crisis: [Name/Description]
Start Date/Time: [Date/Time]
Status: [Active/Resolved/Recovering]
Severity: [Level 1/2/3]

TIMELINE:

[Date/Time] - [Event/Decision/Action]
• Details: [What happened]
• Who: [Person/Team involved]
• Impact: [What was affected]
• Decision: [What was decided]

[Date/Time] - [Event/Decision/Action]
• Details: [What happened]
• Who: [Person/Team involved]
• Impact: [What was affected]
• Decision: [What was decided]

[Continue for all key events]

CURRENT STATUS:
[Current situation summary]

NEXT ACTIONS:
• [Action 1] - Owner: [Name] - Due: [Time]
• [Action 2] - Owner: [Name] - Due: [Time]

KEY DECISIONS:
• [Decision 1] - Made by: [Name] - Rationale: [Why]
• [Decision 2] - Made by: [Name] - Rationale: [Why]
```

-----

## Crisis Communication Templates

### Internal Communication Template (All-Employees)

```
Subject: [Important] Company Update - [Brief Description]

Team,

I'm writing to inform you about [brief description of situation].

WHAT HAPPENED:
[2-3 sentences with facts only, no speculation]

CURRENT STATUS:
[What we know right now, what's under control]

WHAT WE'RE DOING:
[Actions being taken, who's leading response]

WHAT THIS MEANS FOR YOU:
[How it affects employees, any changes to operations]

WHAT YOU SHOULD DO:
• [Specific action if any]
• [Where to get updates]
• [Who to contact with questions]

We'll provide updates [frequency]. Thank you for your patience and professionalism as we work through this.

[Name]
[Title]
```

### External Communication Template (Customers)

```
Subject: Important Update About [Issue]

Dear [Customer Name],

We're writing to inform you about [brief issue description] that may have affected your account.

WHAT HAPPENED:
[Clear, factual description in plain language]

WHAT DATA WAS AFFECTED:
[Specific types of information, be transparent]

WHAT WE'RE DOING:
• [Action 1]
• [Action 2]
• [Action 3]

WHAT YOU SHOULD DO:
[Clear, actionable steps for customers]

WHAT WE'RE OFFERING:
[Any remediation: credit monitoring, compensation, etc.]

HOW TO GET HELP:
• Visit: [URL]
• Email: [dedicated email]
• Phone: [dedicated hotline]

We sincerely apologize for this incident. Your trust is paramount to us, and we're committed to making this right.

[Name]
[Title]
[Company]
```

### Media Holding Statement Template

```
HOLDING STATEMENT
For Immediate Release

[Company] is aware of [incident description]. We are actively investigating the situation and taking immediate steps to address it.

The safety and security of our [customers/employees/users] is our top priority. We are working closely with [relevant authorities/experts] to understand the full scope and impact.

We will provide updates as we have more information to share.

For more information:
[Spokesperson Name]
[Title]
[Email]
[Phone]
```

-----

## Post-Crisis Review Template

```
POST-CRISIS REVIEW
─────────────────────────────────────

Crisis: [Name/Description]
Date: [Date of crisis]
Review Date: [Date of review]
Review Team: [Names, roles]

1. CRISIS OVERVIEW
─────────────────────────────────────
What happened:
[2-3 sentence summary]

Timeline:
[Key events with timestamps]

Impact:
• Customer impact: [Description]
• Business impact: [Description]
• Reputation impact: [Description]

2. RESPONSE ASSESSMENT
─────────────────────────────────────
What Went Well:
• [Item 1]
• [Item 2]
• [Item 3]

What Didn't Work:
• [Item 1]
• [Item 2]
• [Item 3]

3. ROOT CAUSE ANALYSIS
─────────────────────────────────────
Why did this happen?
[Primary root cause]

Contributing factors:
• [Factor 1]
• [Factor 2]
• [Factor 3]

4. LESSONS LEARNED
─────────────────────────────────────
Key Takeaways:
• [Learning 1]
• [Learning 2]
• [Learning 3]

5. ACTION ITEMS
─────────────────────────────────────
• [Action] - Owner: [Name] - Due: [Date]
• [Action] - Owner: [Name] - Due: [Date]
• [Action] - Owner: [Name] - Due: [Date]

6. PLAN UPDATES
─────────────────────────────────────
What will we change in our crisis management plan?
• [Change 1]
• [Change 2]
• [Change 3]

NEXT REVIEW: [Date]
```

-----

## Crisis Management Team Contact List Template

```
CRISIS MANAGEMENT TEAM CONTACT LIST
─────────────────────────────────────

Last Updated: [Date]
Next Review: [Date]

CORE TEAM:

Crisis Manager:
• Primary: [Name] - [Phone] - [Email]
• Backup: [Name] - [Phone] - [Email]

Crisis Coordinator:
• Primary: [Name] - [Phone] - [Email]
• Backup: [Name] - [Phone] - [Email]

Technical Lead:
• Primary: [Name] - [Phone] - [Email]
• Backup: [Name] - [Phone] - [Email]

Communications Lead:
• Primary: [Name] - [Phone] - [Email]
• Backup: [Name] - [Phone] - [Email]

Legal Counsel:
• Primary: [Name] - [Phone] - [Email]
• Backup: [Name] - [Phone] - [Email]

People Lead:
• Primary: [Name] - [Phone] - [Email]
• Backup: [Name] - [Phone] - [Email]

EXTENDED TEAM:

Security Lead: [Name] - [Phone] - [Email]
Finance Lead: [Name] - [Phone] - [Email]
Operations Lead: [Name] - [Phone] - [Email]
Customer Success Lead: [Name] - [Phone] - [Email]

EXTERNAL RESOURCES:

Legal Firm: [Name] - [Phone] - [Email]
PR Firm: [Name] - [Phone] - [Email]
Security Firm: [Name] - [Phone] - [Email]

CRISIS COMMUNICATION CHANNELS:
• Primary: [Slack channel/Phone bridge]
• Backup: [Alternative channel]
• Command Center: [Location/Zoom room]
```

-----

## Template Usage Tips

1. **Customize immediately** - Fill in your organization's specific information
2. **Keep updated** - Review contact lists monthly, plans quarterly
3. **Practice** - Use templates in crisis simulations
4. **Version control** - Track changes to templates over time
5. **Accessibility** - Ensure templates are accessible to all CMT members

-----

## References

- Crisis Management Framework: `1-crisis-management-framework.md`
- Crisis Management README: `README.md`
- Escalation: `../2.2.5-Escalation/README.md`
- Stakeholder Management: `../2.2.7-Stakeholder-Management/README.md`
- Self-Reflection: `../../2.9-Other/2.9.2-Self-Reflection/README.md`

